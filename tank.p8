...
function updateplane()
 ...
 if((plflip and plx > pltarget-(3*plspeed) and plx < pltarget-plspeed)
   or (not plflip and plx < pltarget+(2.5*plspeed) and plx < pltarget)) then
  plopen = true
  plbtime -= 1
  local bsp = plspeed
  if(not plflip) bsp *= -1
  if(plbtime < 1) plbtime = 30 addbomb(plbayx + 2, ply + 16, bsp, 0, plflip, 2)
 end
end
...
function drawmap()
 ...
 for rock in all(rockmap) do --pull in rock table and draw each rock
  if(rock.y > heightmap[rock.x]) then
   ...
   if rock.y == 111 then
    if(dino==0) sspr(0, 56, 16, 8, rock.x, rock.y, 16, 8)--only draw dino once
	dino=1
   else
